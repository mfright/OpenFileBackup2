...
public class destinationSelector extends JDialog {
	 ...
	 * Create the dialog.
	 */
	public destinationSelector(UserInterface ui,JTextField targetBox,JLabel targetId,JLabel targetPw) {
		setTitle("\u30D0\u30C3\u30AF\u30A2\u30C3\u30D7\u306E\u4FDD\u5B58\u5148");
		...
		getContentPane().add(buttonPane, BorderLayout.SOUTH);
		{
			...
			okButton.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					//設定値を親ウィンドウへ書き戻す
					if(rdbFtp.isSelected()){
						targetBox.setText("ftp://"+txtServerUrl.getText());
						targetId.setText(txtFtpid.getText());
						targetPw.setText(txtFtppw.getText());
					}else{
						targetBox.setText(txtStoragePath.getText());
					}
					dispose();
				}
			});
		...
		txtFtpid.setText(targetId.getText());
		...
		txtFtppw.setText(targetPw.getText());
		...
		// 設定されたパスを表示
		if(targetBox.getText().indexOf("ftp://")==0){
			rdbFtp.setSelected(true);
			rdbStorage.setSelected(false);
			txtServerUrl.setText(targetBox.getText().substring(6));
		}else{
			txtStoragePath.setText(targetBox.getText());
		}
